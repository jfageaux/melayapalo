    ...
    <section className="bg-void py-24 lg:py-32 px-6">
      <div className="max-w-7xl mx-auto">
        ...
        <div className="grid sm:grid-cols-2 lg:grid-cols-4 gap-px bg-ash/8">
          {weeks.map((w) => (
            <div key={w.label} className="group bg-void p-8 flex flex-col hover:bg-smoke transition-colors">
              <div className={`w-8 h-0.5 mb-6 origin-left transition-transform group-hover:scale-x-150 ${w.accentClass}`} />
              ...
              <p className="font-body text-ash/55 text-sm leading-relaxed mb-5">{w.focus}</p>
              <span className="font-body text-dust text-xs tracking-[0.12em] mt-auto">{w.sessions}</span>
            </div>
          ))}
        </div>

        <p className="font-body text-ash/50 text-sm mt-8 text-center">
